...
public class WebProtegeIRIShortFormProvider_TestCase {
	...
	public void setUp() throws Exception {
		...
		when(annotationAssertionAxiomsProvider.getAnnotationAssertionAxioms(iri)).thenReturn(Sets.newHashSet(annotationAssertion));
		shortFormProvider = new WebProtegeIRIShortFormProvider(DefaultShortFormAnnotationPropertyIRIs.asImmutableList(), annotationAssertionAxiomsProvider, languageProvider);
	}
	...
	@Test
	public void shouldReturnBuiltInRenderingFor_Custom_Prefix() {

		Enumeration<String> en = ResourceBundle.getBundle("prefixes").getKeys();
		while (en.hasMoreElements()) {
			String e = en.nextElement();
			String uri = ResourceBundle.getBundle("prefixes").getString(e);
			...
			if (!exists){
				IRI iri = IRI.create(uri + "suffix");
				assertThat(shortFormProvider.getShortForm(iri), equalTo(e + ":suffix"));
